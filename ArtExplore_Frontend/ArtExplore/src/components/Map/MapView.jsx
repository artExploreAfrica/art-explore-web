...
const MapView = () => {
    ...
    map.on('load', async () => {
      ...
      map.on('click', 'landmark-icon', (e) => {
        ...
        new maplibregl.Popup({ offset: [0, -8], maxWidth: '240px' })
          ...
          .setHTML(`<div class="landmark-popup"><span class="landmark-popup__icon">📍</span><div><strong>${p.name}</strong><p>${p.description || 'Landmark'}</p></div></div>`)
          .addTo(map);
      });
  ...
  }, []); // galleries is module-level constant — no need in deps array
