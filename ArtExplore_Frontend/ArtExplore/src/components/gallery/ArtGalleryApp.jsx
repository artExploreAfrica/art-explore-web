...
// FIXED: was broken — had a stray `return;` before the expression
...
function useGalleryFilters(data) {
  ...
  return {
    ...
    setSortBy,
    nearMe,
    toggleNearMe: () => setNearMe((p) => !p),
    openNow,
    toggleOpenNow: () => setOpenNow((p) => !p),
    topRated,
    toggleTopRated: () => setTopRated((p) => !p),
    hasActiveFilters,
    resetAllFilters,
  ...
  return (
    <div className="filter-row">
      ...
      <div className="filter-row__right">
        <button className={`chip ${nearMe ? "chip--on" : ""}`} onClick={toggleNearMe}>
          Near Me
        </button>
        <button className={`chip ${openNow ? "chip--on" : ""}`} onClick={toggleOpenNow}>
          Open Now
        </button>
        <button className={`chip ${topRated ? "chip--on" : ""}`} onClick={toggleTopRated}>
          Top Rated
        </button>
  ...
  return (
    <div className="gallery-card">
      ...
      <div className="card__body">
        ...
        {gallery.address && <p className="card__address">{gallery.address}</p>}
        ...
        <div className="card__rating">
          <div className="card__stars">
            {[1, 2, 3, 4, 5].map((n) => (
              <span key={n} className={`card__star ${n <= Math.round(gallery.rating) ? "card__star--on" : ""}`}>
                ★
              </span>
            ))}
          </div>
